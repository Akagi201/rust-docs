...
如[Polkadot 白皮书](https://polkadot.network/PolkaDotPaper.pdf)所述，parachain 模型试图缓解目前技术栈的五个关键构建故障。
...
### [共享安全 (Pooled Security)](https://wiki.polkadot.network/docs/learn-security)
...
### [链上治理 (Thought-through Governance)](https://wiki.polkadot.network/docs/learn-governance)

区块链中的大多数治理系统使用的是链外治理机制。Polkadot 的链上治理鼓励代币持有人的最大参与，并且是无摩擦和透明的。它还可以实现[无障碍升级](https://wiki.polkadot.network/docs/learn-runtime-upgrades)。
...
有了 Polkadot，如果你把你的应用程序构建为一个 parachain，你将会得到这个功能的开箱即用。[XCM 格式](https://wiki.polkadot.network/docs/learn-crosschain)允许任何 parachain 通过在它们之间传递消息进行通信。此外，随着连接到其他链的桥梁（如比特币或以太坊的桥梁），Polkadot 的 parachains 也将能够与这些链通信。

> #### 注意
>
> 尽管成为 parachain 有很多好处，开发者应该意识到成为 parachain 的挑战，以及建立一个以成为 parachain 为最终目标的区块链对他们的项目来说是否可行。

在 Polkadot 上，你能够把你的区块链的最新区块头放到中继链上。作为一个 parachain，你提交的区块由具有 Wasm 运行时的验证者进行验证，可以存储在中继链上。你还可以使用[XCM 格式](https://wiki.polkadot.network/docs/learn-crosschain)与其他 parachain 进行通信：一个抽象的消息传递系统。消息传递在中继链上被跟踪 -- 因此，你可以证明消息的传递，并促进无信任的互动。
...
### [平行链经济 (Para-nomics)](https://wiki.polkadot.network/docs/learn-parachains#parachain-economies)
...
[收集人](https://wiki.polkadot.network/docs/learn-collator)作为网络维护者，维护一个完整 parachain 的节点。他们的激励是由原生代币支付的。
...
>
> Polkadot 网络将鼓励不同的 para-objects 之间的连接和互操作性。
> 这里，para-objects 指的是网络上平行运行的对象，一般来说，是可并行的对象。

这些可以是下面的形式：

* 系统级链（永久链）：[租借槽](https://wiki.polkadot.network/docs/learn-auction)，[平行线程池](https://wiki.polkadot.network/docs/learn-parathreads)
* [桥接](https://wiki.polkadot.network/docs/learn-bridges)枢纽
* 嵌套的中继链：[Polkadot 2.0](https://wiki.polkadot.network/docs/learn-launch#polkadot-20)
...
例如，目前通过在最新的插槽拍卖中获得插槽的网络，可以选择在 Kusama 上部署[智能合约](https://wiki.polkadot.network/docs/build-smart-contracts)。

## 实现一条平行链

Parachain 实现者指南是一项正在进行的重要工作，由 Parity Tech 维护。[实时版本](https://w3f.github.io/parachain-implementers-guide/)是由位于[官方 Polkadot 仓库](https://github.com/paritytech/polkadot/tree/master/roadmap/implementers-guide)中的源代码构建的。
...
收集人节点是协议中网络维护者的类型之一。他们负责保持 parachain 的状态和从状态转换函数的迭代中返回的新状态的可用性。它们必须保持在线，以跟踪状态，也跟踪它将在自己和其他 parachain 之间路由的 XCMP 消息。收集人节点负责将简洁的证明传递给中继链的验证者，并跟踪中继链的最新块。实质上，收集人节点也充当了中继链的轻型客户端。更多关于收集人节点的信息，请参见[收集人页面](https://wiki.polkadot.network/docs/learn-collator)。
...
> ##### Substrate 入门
>
> 开始使用 Substrate 的最好方法是探索[Substrate 开发者中心](https://docs.substrate.io/)，这是一个由[Parity Technologies](https://www.parity.io/)建立和维护的在线资源。
...
关于 Cumulus 的更详细的描述，请参见[Cumulus 概述](https://github.com/paritytech/cumulus/blob/master/docs/overview.md)，对于那些有 Substrate 经验的人来说，可以尝试一下[Cumulus 教程](https://docs.substrate.io/tutorials/v3/cumulus/start-relay/)。

Cumulus 仍在开发中，但它的想法是，通过导入 `crates` 并添加一行代码，应该可以简单地采用 Substrate 链并添加 parachain 代码。从[Cumulus 部分](https://wiki.polkadot.network/docs/build-pdk#cumulus)了解最新的 Cumulus 发展。

> ##### 注意
>
> Substrate 和 Cumulus 从区块链格式的抽象中提供了一个 PDK，但是一个 parachain 甚至不需要是一个区块链。例如，一个 parachain 只需要满足上面列出的两个约束：状态转换功能和收集人节点。
> 其他一切都由 PDK 的实现者决定。
...
你对构建 PDK 感兴趣吗？请参见[未来的 PDK 部分](https://wiki.polkadot.network/docs/build-pdk#future-pdks)，了解详情。
...
> #### 注意
>
> 你想从头开始建立一个 Parachain 开发工具包吗？Web3 基金会正在为从事这项工作的团队提供资助，了解更多信息并在[W3F 资助页面](https://grants.web3.foundation/)申请。

W3F 有兴趣支持的 PDK 的一个例子是一个[roll-up 工具包](https://ethresear.ch/t/roll-up-roll-back-snark-side-chain-17000-tps/3675)，它允许开发者创建基于 SNARK 的 parachains。如果我们回顾一下 roll-up write-up，我们会发现系统使用了两个角色：更新状态的用户和将状态更新聚合到一个单一链上更新的操作者。我们应该可以直接看到如何将其转化为平行链术语。roll-up-like 的 parachain 的状态转换函数将从用户的输入中更新状态（在实践中，很可能是 Merkle 树，这将很容易被验证）。操作员将充当收集人节点，它将汇总状态并创建 zk-SNARK 证明，它将交给中继链的验证人进行验证。

如果你或你的团队对开发 PDK 感兴趣，请随时在[W3F 协作库](https://github.com/w3f/Grants-Program/)中开辟一个问题以征求意见。这种类型的工作可能会有补助。

## 测试平行链

### Rococo 测试网

[Rococo](https://github.com/paritytech/cumulus#rococo-)是一个为测试 parachains 而建立的测试网。Rococo 利用 Cumulus 和[HRMP](https://wiki.polkadot.network/docs/learn-crosschain#xcmp-lite-hrmp)（Horizontal Relay-routed Message Passing）在 parachains 和 Relay Chain 之间发送转账和信息。每条消息都被发送到中继链，然后从中继链发送到所需的 parachain。

Rococo 目前运行着四个测试系统的平行链：[Statemint Tick](https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Fstatemint-rococo-rpc.parity.io#/explorer), [Tick](https://polkadot.js.org/apps/?rpc=wss://trick-rpc.polkadot.io#/explorer) 和 [Track](https://polkadot.js.org/apps/?rpc=wss://track-rpc.polkadot.io#/explorer)。以及几个外部开发的 parachain。

### 现在在 Rococo 上平行链是什么？

你可以在[这里](https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Frococo-rpc.polkadot.io#/parachains)看到所包括的 parachains 的列表。在[这里](https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Frococo-rpc.polkadot.io#/parachains/proposals)可以看到被提议的 parachains 清单。

### 获取 ROC

ROC 可以在 Matrix 上的 [Rococo Faucet](https://app.element.io/#/room/#rococo-faucet:matrix.org) 频道获得。要接收 ROC 代币，请使用以下命令：
...
如果你对运行和启动你自己的 parathread 或 parachain 感兴趣，Parity Technologies 已经创建了一个[Cumulus 教程](https://docs.substrate.io/tutorials/v3/cumulus/start-relay/)来告诉你如何做。在这一过程中遇到困难或需要支持？加入[Parachain 技术 Matrix 聊天频道](https://matrix.to/#/#parachain-technical:matrix.parity.io)，在那里与其他建设者联系。
...
![rococo_downward_transfer](assets/rococo_downward_transfer.png)
...
![rococo_upward_transfer](assets/rococo_upward_transfer.png)
...
![hrmp_alice_send](assets/hrmp_alice_send.png)

Alice 现在能够从她在 parachain 200 的账户向她在 parachain 300 的账户汇款。

![hrmp_alice_send_parachain](assets/hrmp_alice_send_parachain.png)

### 如何连接到一个平行链

如果你想通过[Polkadot-JS Apps](https://polkadot.js.org/apps/#/)连接到 parachain，你可以通过点击导航左上角的网络选择，选择任何 parachain。

![polkadotjs_network_parachains](assets/polkadotjs_network_parachains.png)

在下面的例子中，我们将使用 Rococo 测试网 "Development" 下的 "Custom Node"，遵循[Cumulus 教程](https://docs.substrate.io/tutorials/v3/cumulus/start-relay/)。

### 平行链 Playground

你也可以利用 Polkadot-JS Apps 提供的账户功能来测试整个 Parachain 的业务流程（例如，众筹、拍卖、注册）。

在[Westend](https://wiki.polkadot.network/docs/maintain-networks#westend-test-network)上启动一个本地节点，运行：
...
基于 Substrate 的链，包括 Polkadot 和 Kusama Relay 链，使用[SS58 编码](https://docs.substrate.io/v3/advanced/ss58/)作为其地址格式。[本页](https://github.com/paritytech/ss58-registry/blob/main/ss58-registry.json)作为典型的注册表，供团队查看哪条链对应于某个前缀，以及哪些前缀是可用的。

### Parachain

要将你的 parachain 纳入 Polkadot 网络，你将需要获得一个 parachain 插槽。

parachain 插槽将在公开拍卖中出售，其机制可以在维基的[平行链拍卖页面](https://wiki.polkadot.network/docs/learn-auction)上找到。

### Parathread

Parathreads 将不需要 parachain 插槽，所以你将不需要参与蜡烛拍卖机制。相反，你将能够把你的 parathread 代码注册到一个中继链上，并从那时起能够开始参与每块拍卖，以将你的状态转换纳入中继链。

更多关于 parathread 每区块拍卖工作的信息，请看更详细的[parathread 页面](https://wiki.polkadot.network/docs/learn-parathreads)。
...
* [The Path of a Parachain Block (Video)](https://www.crowdcast.io/e/polkadot-path-of-a-parachain-block)
* [Polkadot Parachain Slots](https://polkadot.network/blog/polkadot-parachain-slots/)
